...
const GameCanvas: React.FC<GameCanvasProps> = ({
    ...
    assetsLoaded, batImage, batsmanImage, ballImage, grassImage, crowdImage
}) => {
    ...
    const drawCrowd = (ctx: CanvasRenderingContext2D, cCrowdImage: HTMLImageElement) => {
        const crowdReady = assetsLoaded.crowd && cCrowdImage.complete && cCrowdImage.naturalWidth > 0;
        if (!crowdReady) return;

        const standWidth = 70;
        const standX = CANVAS_WIDTH - standWidth - 10;
        const standYStart = 20;
        const standYEnd = CANVAS_HEIGHT - 20;
        
        // Draw the stand area
        ctx.fillStyle = 'rgba(0,0,0,0.3)';
        ctx.fillRect(standX, standYStart, standWidth, standYEnd - standYStart);
        ctx.strokeStyle = '#2d3748';
        ctx.strokeRect(standX, standYStart, standWidth, standYEnd - standYStart);

        // Populate with fans
        const fanSize = 24;
        const rows = Math.floor((standYEnd - standYStart) / fanSize);
        const cols = 2;
        
        for(let r = 0; r < rows; r++) {
            for(let c = 0; c < cols; c++) {
                const fx = standX + 5 + (c * (fanSize + 5));
                const fy = standYStart + 5 + (r * (fanSize + 2));
                // Slight offset for organic feel
                const offset = Math.sin(Date.now() / 200 + r) * 2;
                ctx.drawImage(cCrowdImage, fx, fy + offset, fanSize, fanSize);
            }
        }
    };
    ...
    }, [ball, batsman, bat, stumps, gameState, shotDirection, assetsLoaded, batImage, batsmanImage, ballImage, grassImage, crowdImage, canvasRef]);
